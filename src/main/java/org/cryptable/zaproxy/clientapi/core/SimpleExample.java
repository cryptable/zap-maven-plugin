...
public class SimpleExample {
	...
	public static void main(String[] args) {
		...
		try {
			...
			org.cryptable.zaproxy.clientapi.core.ApiResponse resp = api.spider.scan(apikey, target, "", "", "");
			String scanid;
			int progress;
			
			// The scan now returns a scan id to support concurrent scanning
			scanid = ((org.cryptable.zaproxy.clientapi.core.ApiResponseElement)resp).getValue();
			
			// Poll the status until it completes
			while (true) {
				Thread.sleep(1000);
				progress = Integer.parseInt(((org.cryptable.zaproxy.clientapi.core.ApiResponseElement)api.spider.status(scanid)).getValue());
			...
			if (! "OK".equals(((org.cryptable.zaproxy.clientapi.core.ApiResponseElement)resp).getValue())) {
			...
			while (true) {
				Thread.sleep(5000);
				progress = Integer.parseInt(((org.cryptable.zaproxy.clientapi.core.ApiResponseElement)api.ascan.status("")).getValue());
